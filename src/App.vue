  ...
  <div class="bg-gray-700 text-white min-h-screen flex items-stretch justify-start">
    ...
    <!-- Main Content -->
     <main class="flex-1 relative">
      <div v-if="activeNote && isEditing" class="px-4 py-8 flex flex-col h-full">
        <input 
          v-model="inputTitle"
          @input="updateNote"
          type="text"
      ...
      <button 
        v-if="isEditing" 
        @click="saveNote"
        class="fixed bottom-6 right-6 bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-full shadow-lg text-lg"
      >
        Save
      </button>
     </main>
